...
def dump(method: str) -> None:
    # str+eval ugliness instead of just taking the function and outputting
    # its __qualname__ so we get the name also when wrapped in @authorized_call
    print("==== %s" % method)
    try:
        pprint.pprint(eval(method)())
...
dump("client.online_update.cancel_downloading")
